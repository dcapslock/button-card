...
| `haptic` | string | none | `success`, `warning`, `failure`, `light`, `medium`, `heavy`, `selection`, `none` | [Haptic feedback](https://companion.home-assistant.io/docs/integrations/haptics/) for Home Assistant Companion app on iOS and Android. This overrides any haptic feedback that Home Assistant may use for the action. Setting to `none` will override Home Assistant haptic for action with no replacement. |
